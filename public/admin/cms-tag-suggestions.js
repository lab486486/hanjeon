/**
 * Frequent categories under the 카테고리 field (max 5).
 * Works with Decap select (benefits) or list widgets.
...
(function () {
  ...
  const HINT_TEXT = "예: 요금할인 → /benefits/ 목록에서 카테고리 필터";
  ...
  async function addTag(field, tag) {
    ...
    try {
      var select = findSelect(field);
      if (select) {
        // Match option by value or visible text
        var matched = "";
        for (var i = 0; i < select.options.length; i++) {
          var opt = select.options[i];
          if (opt.value === tag || (opt.textContent || "").trim() === tag) {
            matched = opt.value;
            break;
          }
        }
        if (!matched) return;
        select.focus();
        setReactInputValue(select, matched);
        select.blur();
        await wait(30);
        return;
      }
