/**
 * Entry form layout for Decap CMS (blogincome UX).
 * Tags ControlContainers via FieldLabel text, then CSS grids:
 *   제목 | 날짜(확인일/업데이트) (6:4)
 *   설명(대상 요약) | 커버 (6:4) when both exist
 *   태그(카테고리) / 퍼머링크 / 본문 full width
 */
(function () {
  var FIELD_RULES = [
    { re: /^제목/, name: "title" },
    { re: /^날짜|^확인일|^업데이트/, name: "date" },
    { re: /^퍼머링크/, name: "slug" },
    { re: /^커버/, name: "cover_image" },
    { re: /^태그|^카테고리/, name: "tags" },
    { re: /^설명|^대상 요약/, name: "description" },
    { re: /^본문/, name: "body" },
  ];
